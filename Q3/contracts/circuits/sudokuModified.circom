...
template sudoku() {
    ...
    component between0and9[9][9];

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            between0and9[i][j] = RangeProof(32);
            between0and9[i][j].range[0] <== 0;
            between0and9[i][j].range[1] <== 9;
            between0and9[i][j].in <== puzzle[i][j];
            assert(between0and9[i][j].out == 1);
